...
export const errorHandler = async (_: FastifyRequest, res: FastifyReply, error: unknown) => {
  ...
  if (err instanceof ZodError) {
    ...
    res.status(HTTP.CODES.InternalServerError).send({ error: 'Internal Server Error' });
  }
