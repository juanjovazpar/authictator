...
export const mfaLoginSchema = userMFACodeSchema.extend(
    userLoginMFASchema.shape,
);
 ...
 * Ensures the password is a string and conforms to custom password rules defined by the `isValidPassword` function. The maximum length allowed is 50 characters.
...
export const passwordSchema = z.object({
    password: z
        .string()
        .trim()
        .max(50, 'Password too long. Max 50 characters long')
        .refine((value: string) => isValidPassword(value), {
            message: sprintf(
                LITERALS.INVALID_PASSWORD_FORMAT,
                `${PASSWORD_RULES}`,
            ),
        }),
});
