...
export const cache = (() => {
  ...
  return {
    ...
    getMFA: (id: string): Promise<string | null> => client.get(`${MFA_CACHE_KEY}:${id}`),

    delMFA: (id: string): Promise<number> => client.del(`${MFA_CACHE_KEY}:${id}`),
    ...
    delLockedLogin: (id: string): Promise<number> => client.del(`${LOCKED_LOGINS_CACHE_KEY}:${id}`),
    ...
    isSessionActive: async (id: string, userId: string): Promise<number> => {
      const cacheKey: string = `sessions:${userId}:${id}`;
